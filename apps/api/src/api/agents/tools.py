...
def get_formatted_item_context(query: str, top_k: int = 5) -> str:
    ...
    """Search the music catalogue and return the top k matching albums, songs, or formats.
...
language. If no brand or model is specified, search broadly rather than refusing.
    ...
    "A summer playlist album for me and a family road trip album"
        -> "Upbeat summer album" | "Family-friendly road trip album"
    "A relaxing jazz album and a classic rock greatest hits"
        -> "Relaxing jazz album" | "Classic rock greatest hits album"
...
def get_formatted_reviews_context(query: str, parent_asins: list[str], top_k: int = 5) -> str:
    ...
    Returns:
        A string of the top k context chunks with IDs prepending each chunk, each representing a review for a given catalogue music item for a given query.
    """
